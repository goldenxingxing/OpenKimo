...
- GitHub Release 中全部安装包的直接下载链接和文件大小。
...
2. Release 同步：在 GitHub `Release` workflow 成功完成后，通过 API 将对应发行版及安装包下载链接复制到 Gitee。
...
5. 在 Gitee Release 正文末尾生成“安装包下载”区，列出附件名称、GitHub 直接下载链接和文件大小。
6. 不向 Gitee 上传二进制附件，避免其社区版单文件 100 MB 配额限制。

该流程设计为可重复执行。重新运行同一标签时会更新正文中的下载区，不产生重复内容。
...
4. Gitee Release 下载区包含 GitHub 的全部附件；
5. 重新运行同步不会产生重复下载区。
